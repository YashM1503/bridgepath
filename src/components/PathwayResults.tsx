...
import { CheckCircle, Star, AlertCircle, Clock, DollarSign, Unlock, Shield, ChevronDown, ChevronUp } from "lucide-react";
...
const SCORE_COLORS: Record<keyof Score, string> = {
  cost: "hsl(175 55% 35%)",
  speed: "hsl(215 65% 40%)",
  access: "hsl(205 75% 48%)",
  risk: "hsl(210 50% 55%)",
};

function confidenceBadge(confidence: number) {
  if (confidence >= 85) return { label: "High confidence", cls: "bp-badge-success" };
  if (confidence >= 70) return { label: "Moderate confidence", cls: "bp-badge-accent" };
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <p className="text-sm text-muted-foreground mb-2">
        Select a pathway to view detailed steps. You can revisit and change your selection at any time.
      </p>
              ...
              <ul className="space-y-0.5">
                {pathway.assumptions.map((a, i) => (
                  <li key={i} className="text-xs text-foreground flex items-start gap-1.5">
                    <CheckCircle size={10} className="mt-0.5 flex-shrink-0 text-success" />
                    {a}
                  </li>
                ))}
              </ul>
                ...
                <ol className="space-y-1.5">
                  {pathway.steps.map((step, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs text-foreground">
                      <span className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold bg-accent text-accent-foreground">
                        {i + 1}
                      </span>
                      {step}
                    </li>
